...
Spawns and monitors AUV services as daemon subprocesses using `multiprocessing`. Supports `start`, `stop`, `start_all`, `stop_all`, and `status`. Managed services: `db`, `hardware_interface`, `camera`. Extend `_SERVICES` and `_TARGETS` to add more.
...
pm.start("db")
pm.start("camera")
pm.status()   # {"db": {"running": True, "pid": 12345}, "camera": {"running": True, ...}, ...}
pm.stop_all()
```
...
| `ZED_CAMERA` | `false` | Enable ZED 2i camera thread |
| `USB_CAMERA` | `false` | Enable USB webcam thread |
| `USB_CAMERA_INDEX` | `0` | `/dev/videoN` index to start scanning from |
| `CAMERA_STREAM_HOST` | `0.0.0.0` | MJPEG server bind address |
| `CAMERA_STREAM_PORT` | `8001` | MJPEG server port |
| `YOLO_MODEL` | `yolov8n.pt` | Path to YOLOv8 weights file |
| `YOLO_CONF` | `0.5` | Minimum detection confidence (0–1) |
...
### `libs/camera_package/` — Camera & Detection

Captures frames from a USB webcam and/or a ZED 2i stereo camera, runs YOLOv8 object detection on each frame, and serves annotated live feeds as MJPEG streams. Detections are written to the DB API automatically. See [docs/camera_package.md](docs/camera_package.md) for the full reference.

```bash
# configure cameras in .env (at least one must be enabled):
# USB_CAMERA=true   USB_CAMERA_INDEX=0
# ZED_CAMERA=true
uv run python -c "from libs.camera_package.camera_manager import run; run()"
# streams at http://localhost:8001
```
...
|---|---|
| [docs/camera_package.md](docs/camera_package.md) | Camera Package — USB/ZED capture, YOLO detection, MJPEG streaming |
